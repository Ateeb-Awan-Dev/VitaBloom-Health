...
const DashboardSettings = () => {
  const [facility, setFacility] = useState({ name: "VitaBloom Health — River Grove Campus", license: "SNF-CA-2024-00342", admin: "Catherine Brooks, MBA", beds: "380" });
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={s.label} className="flex items-center justify-between p-3 rounded-lg bg-muted/30">
                ...
                <span className="text-xs px-2 py-0.5 rounded-full bg-vitabloom-success/10 text-vitabloom-success">{s.status}</span>
              </div>
            ))}
          </div>
        </div>
            ...
            {integrations.map((int, i) => (
              <div key={int.name} className="flex items-center justify-between p-3 rounded-lg bg-muted/30">
                ...
                <button onClick={() => toggleIntegration(i)} className={`text-xs px-3 py-1 rounded-full cursor-pointer transition-colors ${
                  int.status === "Disconnected" ? "bg-destructive/10 text-destructive hover:bg-destructive/20" : "bg-vitabloom-success/10 text-vitabloom-success hover:bg-vitabloom-success/20"
                }`}>{int.status}</button>
              </div>
            ))}
          </div>

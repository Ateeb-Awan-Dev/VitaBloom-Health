        ...
        {shifts.map((s) => (
          <div key={s.id} className={`${card} p-5`}>
            <div className="flex items-center justify-between mb-2">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full ${s.status === "Active" ? "bg-vitabloom-success/10 text-vitabloom-success" : "bg-muted text-muted-foreground"}`}>{s.status}</span>
            </div>
      ...
      {conflicts.length > 0 && (
        <div className={`${card} p-5`}>
          <h3 className="text-sm font-semibold text-foreground mb-3 font-sans flex items-center gap-2">
            <AlertTriangle className="w-4 h-4 text-vitabloom-warning" />Scheduling Alerts
          </h3>
          <div className="space-y-2">
            {conflicts.map((c) => (
              <div key={c.issue} className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
                <span className={`w-2 h-2 rounded-full shrink-0 ${c.severity === "Critical" ? "bg-destructive" : c.severity === "Urgent" ? "bg-vitabloom-warning" : "bg-vitabloom-info"}`} />
                <p className="text-sm text-foreground">{c.issue}</p>
                <span className={`text-xs px-2 py-0.5 rounded-full ml-auto ${c.severity === "Critical" ? "bg-destructive/10 text-destructive" : c.severity === "Urgent" ? "bg-vitabloom-warning/10 text-vitabloom-warning" : "bg-vitabloom-info/10 text-vitabloom-info"}`}>{c.severity}</span>
              </div>
            ))}
          </div>
        </div>

          ...
          {careTasks.map((t) => (
            <div key={t.id} className="flex items-center gap-4 p-3 rounded-lg bg-muted/30 hover:bg-muted/50 transition-colors">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full ${
                ...
                t.priority === "Medium" ? "bg-vitabloom-warning/10 text-vitabloom-warning" :
                "bg-muted text-muted-foreground"
              }`}>{t.priority}</span>
              <span className={`text-xs px-2 py-0.5 rounded-full ${
                t.status === "Completed" ? "bg-vitabloom-success/10 text-vitabloom-success" :
                t.status === "In Progress" ? "bg-vitabloom-info/10 text-vitabloom-info" :
                "bg-muted text-muted-foreground"
              }`}>{t.status}</span>
            </div>
          ))}
            ...
            {fallRiskAssessments.map((f) => (
              <div key={f.resident} className="p-3 rounded-lg bg-muted/30">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${f.level === "High" ? "bg-destructive/10 text-destructive" : "bg-vitabloom-warning/10 text-vitabloom-warning"}`}>
                    Score: {f.score}
                  </span>
                </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          {conditions.map((c) => (
            <div key={c.name} className="p-4 rounded-lg bg-muted/30">
              ...
              <span className={`text-xs mt-2 inline-block ${c.trend === "Improving" ? "text-vitabloom-success" : "text-muted-foreground"}`}>{c.trend}</span>
            </div>
          ))}
        </div>
      </div>

  ...
  <div className="space-y-6">
    ...
    <div className={`${card} p-6`}>
      <h3 className="text-sm font-semibold text-foreground mb-4 font-sans flex items-center gap-2">
        <AlertTriangle className="w-4 h-4 text-vitabloom-warning" />Drug Interaction & Allergy Alerts
      </h3>
      <div className="space-y-2">
        {alerts.map((a) => (
          <div key={a.desc} className={`flex items-center gap-3 p-3 rounded-lg ${a.severity === "Critical" ? "bg-destructive/5 border border-destructive/20" : "bg-vitabloom-warning/5 border border-vitabloom-warning/20"}`}>
            <Shield className={`w-4 h-4 shrink-0 ${a.severity === "Critical" ? "text-destructive" : "text-vitabloom-warning"}`} />
            <div className="flex-1">
              <p className="text-xs font-medium text-foreground">{a.type}</p>
              <p className="text-xs text-muted-foreground">{a.desc}</p>
            </div>
            <span className={`text-xs px-2 py-0.5 rounded-full ${a.severity === "Critical" ? "bg-destructive/10 text-destructive" : "bg-vitabloom-warning/10 text-vitabloom-warning"}`}>{a.severity}</span>
          </div>
        ))}
      </div>
    </div>
                ...
                <tr key={i} className="border-b border-border/50">
                  ...
                  <td className="py-2.5">
                    <span className={`text-xs px-2 py-0.5 rounded-full ${
                      r.status === "Administered" ? "bg-vitabloom-success/10 text-vitabloom-success" :
                      r.status === "Refused" ? "bg-destructive/10 text-destructive" :
                      "bg-vitabloom-warning/10 text-vitabloom-warning"
                    }`}>{r.status}</span>
                  </td>
      ...
      <div className="space-y-2">
        {pharmacyOrders.map((o) => (
          <div key={o.item} className="flex items-center gap-4 p-3 rounded-lg bg-muted/30">
            ...
            <span className={`text-xs px-2 py-0.5 rounded-full ${
              o.status === "Shipped" ? "bg-vitabloom-success/10 text-vitabloom-success" :
              o.status === "Processing" ? "bg-vitabloom-info/10 text-vitabloom-info" :
              "bg-vitabloom-warning/10 text-vitabloom-warning"
            }`}>{o.status}</span>

...
import {
  Flower2, LayoutDashboard, Users, Heart, CalendarDays, Pill,
...
const DashboardLayout = () => {
  ...
  useEffect(() => {
    const auth = localStorage.getItem("vitabloom_auth");
    if (!auth) navigate("/staff-login");
  }, [navigate]);

  const handleLogout = () => {
    localStorage.removeItem("vitabloom_auth");
    navigate("/");
  };
    ...
    <div className="flex flex-col h-full">
      <div className="p-4 flex items-center gap-2.5 border-b border-sidebar-border">
        <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-sidebar-primary to-primary flex items-center justify-center shrink-0">
          <Flower2 className="w-4 h-4 text-sidebar-primary-foreground" />
        </div>
        {!collapsed && <span className="text-sm font-bold text-sidebar-foreground font-display">VitaBloom</span>}
      </div>

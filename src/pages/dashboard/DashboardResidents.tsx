          ...
          <motion.div key={r.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className={`${card} p-5 cursor-pointer hover:shadow-md transition-shadow`}>
            <div className="flex items-start justify-between mb-3">
              ...
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${
                ...
                r.status === "Moderate" ? "bg-vitabloom-warning/10 text-vitabloom-warning" :
                r.status === "Improving" ? "bg-vitabloom-info/10 text-vitabloom-info" :
                "bg-vitabloom-success/10 text-vitabloom-success"
              }`}>{r.status}</span>
            </div>

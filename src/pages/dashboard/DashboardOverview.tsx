...
const metrics = [
  ...
  { label: "Wellness Score", value: "87.4", change: "+2.1", trend: "up", icon: Activity, color: "text-vitabloom-success" },
  { label: "Fall Incidents (30d)", value: "4", change: "-3", trend: "down", icon: AlertTriangle, color: "text-vitabloom-warning" },
  { label: "Med Adherence", value: "98.2%", change: "+0.4%", trend: "up", icon: Pill, color: "text-vitabloom-info" },
...
const DashboardOverview = () => (
  <div className="space-y-6">
    <div>
      ...
      <p className="text-sm text-muted-foreground mt-1">VitaBloom Health · Facility Management Console</p>
    </div>
      ...
      {metrics.map((m, i) => (
        <motion.div key={m.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className={`${card} p-5`}>
          <div className="flex items-center justify-between mb-3">
            ...
            <span className="text-xs font-medium flex items-center gap-0.5 text-vitabloom-success">
        ...
        <div className="space-y-3">
          {highRiskResidents.map((r) => (
            <div key={r.name} className="flex items-start justify-between p-3 rounded-lg bg-muted/50">
              ...
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${r.risk === "High" ? "bg-destructive/10 text-destructive" : "bg-vitabloom-warning/10 text-vitabloom-warning"}`}>{r.score}</span>
            </div>
          ))}
        </div>
      </div>
          ...
          {todayActivities.map((a) => (
            <div key={a.time} className="flex items-center gap-3 p-3 rounded-lg bg-muted/30">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full ${
                a.status === "completed" ? "bg-vitabloom-success/10 text-vitabloom-success" :
                a.status === "in-progress" ? "bg-vitabloom-info/10 text-vitabloom-info" :

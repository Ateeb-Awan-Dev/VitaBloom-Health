    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {[
        ...
        <div key={s.label} className={`${card} p-5`}>
          ...
          {s.change && <p className="text-xs text-vitabloom-success mt-1">{s.change}</p>}
        </div>
      ))}
    </div>
    ...
    <div className={`${card} p-6`}>
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => {
          const doc = new jsPDF();
          doc.setFontSize(18);
          doc.text("VitaBloom Health — Invoice Report", 14, 20);
          doc.setFontSize(10);
          ...
          autoTable(doc, {
            ...
            styles: { fontSize: 9 },
            headStyles: { fillColor: [34, 118, 106] },
          });
          doc.save("VitaBloom_Invoices.pdf");
            ...
            {recentInvoices.map((inv) => (
              <tr key={inv.id} className="border-b border-border/50">
                ...
                <td className="py-2.5"><span className={`text-xs px-2 py-0.5 rounded-full ${
                  inv.status === "Paid" ? "bg-vitabloom-success/10 text-vitabloom-success" :
          ...
          {insuranceClaims.map((c) => (
            <div key={c.claim} className="flex items-center gap-4 p-3 rounded-lg bg-muted/30">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full ${
                c.status === "Paid" || c.status === "Approved" ? "bg-vitabloom-success/10 text-vitabloom-success" :
                c.status === "Under Review" ? "bg-vitabloom-info/10 text-vitabloom-info" :
                "bg-vitabloom-warning/10 text-vitabloom-warning"
              }`}>{c.status}</span>
            </div>
          ))}

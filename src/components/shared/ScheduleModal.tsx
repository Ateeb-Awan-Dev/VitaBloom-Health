...
const ScheduleModal = () => {
  ...
  useEffect(() => {
    const shown = localStorage.getItem("vitabloom_modal_shown");
    if (shown) return;
    const timer = setTimeout(() => {
      setOpen(true);
      localStorage.setItem("vitabloom_modal_shown", "true");
    }, 5000);
      ...
      {open && (
        ...
        >
          ...
          >
            ...
            <div className="w-14 h-14 rounded-2xl logo-tile-gradient flex items-center justify-center mb-6 shadow-md shadow-primary/15">
              <Calendar className="w-7 h-7 text-primary-foreground" />
            </div>
            <h3 className="text-2xl font-bold text-foreground mb-2">Short call, clear next steps</h3>
            <p className="text-muted-foreground text-sm mb-6 leading-relaxed">
              A complimentary 15-minute conversation with a VitaBloom solutions lead — we listen first, then show only what fits your campuses.
            </p>
            <div className="space-y-3">
              ...
              <button onClick={handleSubmit} className="w-full flex items-center justify-center gap-2 px-6 py-4 rounded-xl btn-gradient-accent text-accent-foreground font-semibold text-sm hover:shadow-lg hover:shadow-primary/20 hover:-translate-y-0.5 transition-all duration-300">
